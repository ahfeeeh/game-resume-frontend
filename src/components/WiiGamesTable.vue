...
export default {
  name: 'WiiGamesTable',
    ...
    const markDlcAsFinished = (idx, id, finished, context) => {
      ...
      modalActiveDLC,
      toggleModalDelete,toggleModalFinished, toggleModalDLC, store, markDlcAsFinished }
    },
  ...
  methods: {        
    getItems(){
      this.store.dispatch('getGames', { payload:{table: 'wii'}, toast: this.toast })
    },
    finishItem(idx, toggleModal) {           
      this.store.dispatch('finishGame', { payload:{idx:idx[0], table: 'wii'}, toast: this.toast })
      toggleModal();   
    },
    deleteItem(idx, toggleModal) {
      this.store.dispatch('deleteGame', { payload:{idx:idx[0], table: 'wii'}, toast: this.toast })  
      toggleModal();  
    },
    editItem(payload) {
      payload.table = 'wii'
