  ...
  <div class="about">
    <h1>Wii Games Table</h1>
     <div class="row align-items-end"  style="margin-top:20px">
        ...
        <div class="col"></div>
        <div class="col">
          <a type="button" class="btn btn-outline-primary" :href="`${env}/csv?table=Wii`">to CSV</a> &nbsp;
          <a type="button" class="btn btn-outline-secondary" :href="`${env}/pdf?from=wii`" target="_blank">to PDF</a> &nbsp;
          <a type="button" class="btn btn-outline-success" :href="`${env}/xls?from=wii`">to XLS</a>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {      
      ...
      isLoading: true,
      newItem: {app_id: "", system_id: 5, title: "", finished: null, collection: null, genuine: null, fisical_disc: null, table: 'wii'}      
    }
  },
  methods: {
    saveGame(payload) {  
      this.store.dispatch('saveGame', { payload, toast: this.toast, toggleModal: this.toggleModal })  
      this.newItem = {app_id: "", system_id: 5, title: "", finished: null, collection: null, genuine: null, fisical_disc: null, table: 'wii'}            
    }
  }
}
