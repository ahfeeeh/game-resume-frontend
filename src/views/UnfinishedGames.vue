...
export default {
  ...
  methods: {
    allUnfinishedGames() {
      const vm = this;
      request(`${process.env.VUE_APP_BACKEND_SERVER}/graphql`, queryAllUnfinished).then(
        (data) => {
          vm.games = data.games;
        }
      );
    ...
    allUnfinishedGamesDetailed() {
      const vm = this;
      request(`${process.env.VUE_APP_BACKEND_SERVER}/graphql`, queryAllUnfinishedDetailed).then(
        (data) => {
          vm.games = data.games;
        }
      );
